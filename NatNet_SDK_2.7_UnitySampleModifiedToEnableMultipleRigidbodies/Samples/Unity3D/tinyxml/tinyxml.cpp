...
const char* TiXmlElement::GetStringAttribute( const char* name ) const
{
...
bool  TiXmlElement::GetBoolAttribute  ( const char* name )
{
    const char *str = Attribute(name);

    if(NULL==str) return false;
    
    if(atoi(str)==1 || strcmp("true",str)==0)
        return true;
    return false;
}

int   TiXmlElement::GetIntAttribute   ( const char* name )
{
    const char *str = Attribute(name);
...
float TiXmlElement::GetFloatAttribute ( const char* name )
{
    const char *str = Attribute(name);
